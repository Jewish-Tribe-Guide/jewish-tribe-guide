 ...
 * with no toggle. Renders nothing when there's no value at all — the caller
 * (PlaceDetailBody) already skips mounting this when the field is unset, so
 * this is a second line of defense, not the primary check.
 *
 * The Google-sync closed badge and "synced … ago" freshness note used to
 * live here, but they're facts about the LISTING (from its placeId), not
 * about any one hours field — a category with no hours field at all
 * (Synagogues, some Schools) still gets synced address/phone/status from
 * Google, and tucking the only place that showed it inside a component that
 * doesn't render without hours meant it never appeared for them. See
