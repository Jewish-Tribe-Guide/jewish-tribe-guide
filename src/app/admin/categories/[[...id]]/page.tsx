...
// is open" as one opaque string it branches on by prefix — 'cat:<id>',
// 'cat:new', 'form:<id>', 'form:new' (see CAT_PREFIX/FORM_PREFIX in
// CategoryManager.tsx) — so that string IS the URL segment here
// (/admin/categories/cat:new, /admin/categories/form:<id>, …).
// CategoryManager's own branching needs no changes at all; only where the
// string comes from (this route param, not AdminTabs' old pushState state)
// and where it goes (router.push, not history.pushState).
//
// 'use client', so params are read via useParams() — NOT the async
// `PageProps<'/admin/categories/[[...id]]'>` + `await props.params`
// convention the public [community]/* Server Components use; that typed
// helper is for async Server Components and doesn't apply to a client page.
