...
import { isAllowedAdminEmail, isAllowedForCommunity } from '@/lib/adminAuth'
...
// with an email that only administers /ues). `community` omitted — as it is
// from the standalone superadmin console at /admin — checks the SUPERADMIN
// list (isAllowedAdminEmail) instead and redirects to /admin itself. Because
// public sign-ups are disabled in Supabase, this server route (service-role)
// is the only path that can create/sign in an admin — so no one else can
// obtain a link by entering an arbitrary email.
export async function POST(request: Request) {
  ...
  if (communitySlug) {
    ...
    redirectTo = `${origin}/${target.slug}/admin`
  } else {
    if (!isAllowedAdminEmail(email)) {
      console.warn(`[admin/request-link] rejected non-superadmin email: ${email}`)
      return genericOk
    }
