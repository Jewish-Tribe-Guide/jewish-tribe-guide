...
// A prior version of this file also covered the home screen's embedded map
// band's own floating search box staying under the Categories mega-menu —
// that band (HomeMap.tsx → ResourceMapView.tsx in its boxed mode) is gone
// (the user's own call: the map only ever lives at its own full-screen route
// now), so there's no longer a same-page z-40 dropdown to race against.
